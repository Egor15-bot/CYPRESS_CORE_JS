describe('Контрагенты', () => {
    ...
    context('Общие кейсы', () => {
        ...
        it('#1366. Фильтрация. Рублевые. Строка поиска', () => { })
        it('#1873. Фильтрация. Рублевые. Фильтрация по типам', () => { })
        it('#3602. Услуга "Светофор" включена - АО "ВМЗ"', () => { })
        it('#3796. Услуга "Светофор" выключена - ООО "БРАИР"', () => { })
        it('#1359. Проверка пагинации', () => { })
        it('#1372. Проверка скорости загрузки страницы "Контрагенты"', () => { })
        
        context('Меню действий "Изменить" - раздел "Контрагенты"', ()=>{
            it('#1971. Контрагент ЮЛ', () => { })
            it('#3797. Контрагент ФЛ с ИНН', () => { })
            it('#3798. Контрагент ФЛ без ИНН', () => { })
            it('#3799. Контрагент ИП', () => { })
            it('#3800. Контрагент НАЛОГ', () => { })
            it('#3801. Контрагент ТАМОЖ', () => { })
        })
        context('Меню действий "Изменить" - раздел "Контрагенты / Просмотр"', ()=>{
            it('#3810. Контрагент ЮЛ', () => { })
            it('#3811. Контрагент ФЛ с ИНН', () => { })
            it('#3812. Контрагент ФЛ без ИНН', () => { })
            it('#3813. Контрагент ИП', () => { })
            it('#3814. Контрагент НАЛОГ', () => { })
    ...
    context('Контрагенты ФЛ с ИНН', () => {
        ...
        context('Меню контрагенты', () => {
            ...
            it('#1905. Создание контрагента + удаление через кебаб-меню', function () {
                ...
                cy.contains('div.counterpart__types__item', 'Физическое лицо').click()
                //Проверяю что выбрана вкладка "Физическое лицо"
                cy.get('div.counterpart__types__item.active')
    ...
    context('Контрагенты ИП', () => {
        ...
        context('Меню контрагенты', () => {
            ...
            it('#1602. Создание контрагента + удаление через кебаб-меню', function () {
                //Нажимаю на вкладку "ИП"
                cy.contains('div.counterpart__types__item', 'ИП').click()
                //Проверяю что выбрана вкладка "ИП"
                cy.get('div.counterpart__types__item.active')
                    .should('contain', 'ИП')
                    .and('have.css', 'border-bottom', `3px solid ${Cypress.env('colorBlueSolid')}`)
                //Заполняю форму тестовыми данными из фикстуры
    ...
    context('Контрагенты НАЛОГ', () => {
        ...
        context('Меню контрагенты', () => {
            ...
            it('#1429. Создание контрагента + удаление через кебаб-меню', function () {
                ...
                cy.contains('div.counterpart__types__item', 'Налоговый/Бюджетный').click()
                //Проверяю что выбрана вкладка "Налоговый/Бюджетный"
                cy.get('div.counterpart__types__item.active')
    ...
    context('Контрагенты ТАМОЖ', () => {
        ...
        context('Меню контрагенты', () => {
            ...
            it('#1422. Создание контрагента + удаление через кебаб-меню', function () {
                //Нажимаю на вкладку "Таможенный"
                cy.contains('div.counterpart__types__item', 'Таможенный').click()
