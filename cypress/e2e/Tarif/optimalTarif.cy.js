describe("", () => {
  beforeEach(() => {
    cy.loginApi();
    cy.changeCompanyApi("6438571");
    cy.visit("/tarif");
  });
  it("#3305 - Оптимальный > Корпоративный ", () => {
  ...
  it("#3332 - Основные положения Оптимальный", () => {
    ...
    cy.get("article.tariff-details-header").within(() => {
      cy.get(".tariff-details-header__title").should("contain", "Оптимальный");
      cy.get(
      ...
      ).should("contain", "20 ₽ за платеж. В адрес ФЛ + доп.комиссия");
      cy.get(
      ...
      ).should("contain", "1 % - при остатке от 130 000 ₽ и более");
